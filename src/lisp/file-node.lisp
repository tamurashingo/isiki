...
;;; dir-lba/dir-offsetは[ファイルI/O]#48(M5)のwrite-from!が、書き込み後に
;;; ディレクトリエントリのsize/start-clusterフィールドを更新するために必要な
;;; 位置情報(このファイルが属するディレクトリの、このエントリ自身が格納されて
;;; いるセクタのLBAとセクタ内オフセット)。パス解決時(%fat16-dir-entry-at等)に
;;; 既に判明している値をそのまま保持させるだけで、書き込みのたびにディレクトリを
;;; 再走査せずに済む。
(defclass <file-node> ()
  ((name :initarg :name :initform nil)
   ...
   (dir-lba :initarg :dir-lba :initform nil)
   (dir-offset :initarg :dir-offset :initform nil)))
...
;;; --- [ファイルI/O]#48(M5): write-from!総称関数の宣言 ---
;;; bufferのbuffer-offset位置からcountバイト分を読み、nodeのfile-offset
;;; バイト目から書き込む(既存のチェイン長を超える場合は新規クラスタを
;;; 確保・連結する)。成功時はt、失敗時はnil。
(defgeneric write-from! (node buffer buffer-offset file-offset count))
