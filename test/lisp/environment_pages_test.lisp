...
;;; --- 4. 複数のdefunを連続してコンパイルしても、g_jit_code(ステージング用
;;;        スクラッチバッファ)の再利用によって互いのコードが破壊されないこと ---
;;
;; [記述の更新 2026-09-15] 以前は「それぞれ別のImmobilized Pageへ再配置され」と
;; 書いていたが、JITコードのパッキング(os_imm_code_alloc)により、**同じ環境の
;; 関数は同じページに詰められる**ようになった。ここで定義する3つは同じ環境なので
;; 同居する。検証したい性質(スクラッチバッファの再利用で壊れないこと)は
;; ページが別かどうかとは独立なので、アサーション自体は変えていない。
;; ページが必ず分かれるのは**環境が違う場合**で、そちらは
;; test/lisp/jit_code_packing_test.lisp が検証する。
