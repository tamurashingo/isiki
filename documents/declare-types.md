...
## 6-2. 性能: テスト全体が 256M で +18%(許容と判断)

`make test-qemu`(QEMU_MEM=256M)の所要時間が増えた。ゲスト内の tick
(`#elapsed`、100 tick = 1秒)で比較する。進捗出力(`#at`)は
`*isiki-test-progress*` で止めて条件を揃えた。

| 構成 | tests | `#elapsed` |
|---|---:|---:|
| 変更前 | 4032 | **18388** |
| 本作業 + `za_fn_meta_t` を 48 byte へ戻す | 4032 | **19348** |
| 本作業(全変更) | 4095 | **21772**(4032 換算 ≒ 21720) |

### 内訳

| 要因 | tick | 秒 | 割合 |
|---|---:|---:|---:|
| **`za_fn_meta_t` の 48 → 64 byte** | **2372** | 約24秒 | **71%** |
| その他(`eval_defun` の走査 / `za_compile_expr` の分岐 / `static inline` 群) | 960 | 約10秒 | 29% |
| 合計 | 3332 | 約33秒 | **+18%** |

`za_compile_let` の declare 走査は**無罪**(無効化しても 21868 で変化なし)。
declare のテスト 63 件自体は 52 tick(0.5秒)しか使っていない。

### なぜ meta のサイズが効くのか

`os_imm_slot_alloc` は `OS_HEAP_ALIGN`(16)へ切り上げ、`IMM_PAGE_SIZE`(4096)から
切り出す。1ページあたり **85 個 → 64 個**になり、**Immobilized Space のページ数が
1.33 倍**になる。処理量ではなく**メモリレイアウトの変化**である。

測定環境は KVM 無しの TCG で、触るページが増えるとソフト MMU のコストが直接効く。
96M では全体で +8% にとどまり、メモリサイズ依存の出方とも整合する。

### 判断: このまま入れる

**増えているのはコンパイル時のコストで、生成コードは 1 バイトも変わらない。**
生成コードが meta から読むのは offset 0(`cons_entry`)と 16(`arity`)だけなので、
実行時に効くとしてもキャッシュ局所性の範囲にとどまる。

コンパイル時間が多少増えるのは許容する、という判断でこのまま入れた。

### 縮めたくなったときの選択肢

`os_imm_slot_alloc` は 16 byte 境界なので、**6 フィールド(48)の次は 64**。
7 フィールドでも 8 フィールドでも丸め後は同じ 64 byte になるため、
**型符号を 1 ワードに詰め直しても効果は無い**。

- **宣言を持つ関数だけ 64 byte で確保する。** 宣言付き関数は稀なので大多数の meta は
  48 byte のまま。確保時点で宣言の有無が要るので `os_make_jit_function` 系に引数を足す。
  **これで 71% ぶんが消え、+18% → +5% 程度になる見込み。**
- 既存フィールドへの相乗りは難しい。`optimize` の bit40-63 は空いているが 24bit しか
  無く、4bit 符号でも 6 変数分で 16 変数に足りない。`arity` は**生成コードが offset 16 を
  直接読む**ので汚せない。
...
**型を使った最適化。** `(+ a b)` の型ガード 5 命令 19 byte を省く
(`documents/type-system-survey.md` §7-2)。ただし
`documents/control-transfer-survey.md` の経緯どおり、`(+ a b)` は PR #70 で
735 byte から縮んでおり、現在の内訳では**プロローグが支配的**である
(fixed entry 171 + cons entry 323)。型ガードを省いて得られる量は相対的に小さい。

**オーバーフロー検査は省けない。** fixnum どうしでも和が範囲を超えれば bignum に
なる。省くと静かに壊れる。
