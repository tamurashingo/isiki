...
- **同じ数字が出たら疑う。** 実行間に 10〜20% のドリフトがあるこの環境では、
  **完全一致のほうが異常**であり、同じバイナリを 2 回測ったとき以外に起こりにくい。
  PR #87 で `git stash` のオプション位置を誤って変更前後に同じバイナリを測ったとき、
  **気づけた唯一の手がかりが「両方が完全に同じ数字だった」ことだった。**
  直感では「同じ数字 = 変化なし = 安定」と読んでしまうが、**ここでは逆である。**
...
> **【追記 / PR #87】この「省ける量は小さい」という見積もりは、TCG 上では
> 過小かもしれない。**
>
> 根拠にしたのは 118 byte という**サイズ**だった。しかし型ガードの実体は
> **分岐 1 本**である。PR #87 で single-float の算術を**分岐ゼロ**にしたところ、
> 命令数がほぼ同じ(9 対 8)fixnum を下回った。**TCG は分岐でトランスレーション
> ブロックが切れるため、分岐 1 本の重みがサイズから推測される以上に大きい**
> (`inline-builtin.md` の `null` は cmov 化で -30%)。
>
> 型駆動の最適化に戻るときは、**「118 byte」ではなく「分岐 1 本」として測り直す**こと。
> ただし実機では分岐予測がよく当たるはずで、TCG ほどの差は出ない可能性が高い
> (`single-float-arith.md` §4-2)。**どちらの環境で測っているかを明示すること。**
